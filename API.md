...
本文档详细说明了 Python 聊天服务器 v9.0 的消息协议和客户端开发指南。
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `type` | string | 是 | 消息类型（text/file_upload/file_request/file_response/heartbeat/user_list） |
| `from` | string | 是 | 发送者昵称 |
| `target` | string | 否 | 目标用户（私聊时使用，默认"所有人"） |
| `msg` | string | 否 | 消息内容（文本消息时必需） |
| `filename` | string | 否 | 文件名 |
| `data` | string | 否 | 文件数据（Base64 编码） |
| `file_id` | string | 否 | 文件唯一标识符 |
| `users` | array | 否 | 在线用户列表 |
...
```json
{
  ...
  "from": "Alice",
  "target": "Bob",
  "msg": "Hi Bob, this is private!"
}
```

**服务器处理：**
- 检查发送者是否被禁言
- 过滤消息内容（长度限制，默认1000字符）
- 如果 target="所有人"，广播给所有其他客户端
- 如果 target=特定用户，仅发送给该用户（O(1)查找）
